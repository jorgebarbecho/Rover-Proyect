...
void loop() 
{
   ...
   // Leer estado de los pulsadores
    ...
    int estado7 = analogRead(JOY_Y_PIN);
        // Reiniciar el tiempo de encendido si se detecta una interacción
    if (estado1 == LOW || estado2 == LOW || estado3 == LOW || estado4 == LOW || estado5 == LOW || estado6 != 540 || estado7 != 540) 
    {
        lcdEncendido = true;
        tiempoEncendido = millis();  // Reiniciar el tiempo de encendido
    }

    // Apagar la pantalla si no hay interacción durante el tiempo definido
    if (lcdEncendido && (millis() - tiempoEncendido > tiempoEncendidoMaximo)) 
    {
        lcdEncendido = false;
        lcd.noBacklight();  // Apagar la pantalla
    }
